...
//import android.app.Activity; Disable THIS!
...
public class AccelHelper {
	...
	//private final Activity activity; //Disable This!
	private final Context context; //CHANGE
	private final Sensor accelerometer;
	private final Sensor magneticField;

	private float accel[] = new float[3] ;
	
	/* REPLACE WITH THE CODES BELLOW THIS ONE
...
*/
	public AccelHelper(Context context) {
		 this.accelerometerValues = new float[3];
	        this.geomagneticMatrix = new float[3];
...
*/
	private static int getDispRotation(Context con) {
		return DispRotateGetter.getInstance().getRotate(((WindowManager) con.getSystemService("window")).getDefaultDisplay());
	}
	...
	private class MySensorListener implements SensorEventListener {
		...
		public void onSensorChanged(SensorEvent e) {
			...
			if (geomagneticMatrix != null && accelerometerValues != null && sensorReady) {
				...
				int dr = AccelHelper.getDispRotation(AccelHelper.this.context);
				float x = 0;
				float y = 0;
				float z = 0;
